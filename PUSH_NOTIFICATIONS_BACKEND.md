...
This document describes the backend implementation required for push notifications when offers are created. The mobile apps (iOS and Android) handle device token registration, but the backend needs to implement the notification sending logic.
...
CREATE TABLE push_tokens (
    ...
    platform VARCHAR(50) NOT NULL, -- 'ios', 'android', or 'web'
...
The mobile apps (iOS/Android) send device tokens to:
...
### 5. Push Notification Services Integration
...
- **Java**: Firebase Admin SDK
- **Node.js**: `firebase-admin` package
- **Python**: `firebase-admin` package

Configure with:
- Firebase project credentials (service account JSON)
- Project ID matching your Android app
